...
class UserLoginBean {
  ...
  UserLoginBean({this.loginType, this.code, this.account, this.token, this.profile, this.bindings, this.cookie});

  UserLoginBean.fromJson(dynamic json) {
    loginType = json['loginType'];
    code = json['code'];
    account = json['account'] == null ? null : UserAccountBean.fromJson(json['account']);
    token = json['token'];
    profile = json['profile'] == null ? null : UserProfileBean.fromJson(json['profile']);
    cookie = json['cookie'];

    if (json['bindings'] != null) {
      bindings = [];
      json['bindings'].forEach((v) {
        bindings?.add(UserBindInfoBean.fromJson(v));
      });
    }
  }
}
